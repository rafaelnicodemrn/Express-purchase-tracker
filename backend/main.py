...
def iniciar_db():
    with sqlite3.connect("compras.db") as conn:
        ...
        if cursor.execute("SELECT COUNT(*) FROM pedidos").fetchone()[0] == 0:
            ...
            conn.executemany("INSERT INTO pedidos (item, quantidade, urgencia, preco_estimado, setor, comprado, data_criacao) VALUES (?,?,?,?,?,?,?)", dados_iniciais)
            conn.commit()
...
async def criar_pedido(pedido: Pedido, background_tasks: BackgroundTasks):
    ...
    with sqlite3.connect("compras.db") as conn:
        ...
        cursor.execute(
            ...
            (pedido.item, pedido.quantidade, pedido.urgencia, pedido.preco_estimado, pedido.setor, False, pedido.data_criacao)
        )
        conn.commit()
        pedido.id = cursor.lastrowid
    
    if (pedido.preco_estimado * pedido.quantidade) > 5000:
        background_tasks.add_task(processar_notificacao_diretoria, pedido.item, pedido.preco_estimado * pedido.quantidade)
...
def listar_pedidos():
    with sqlite3.connect("compras.db") as conn:
        ...
        pedidos = conn.execute("SELECT * FROM pedidos ORDER BY comprado ASC, id DESC").fetchall()
